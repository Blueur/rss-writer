 ...
 */
function xmlLine(markup, content, indentation, attributes) {
  if (attributes) {
    return indentation + "<" + markup + " " + attributes + " ><![CDATA[" + content + "]]></" + markup + ">\n"
  } else {
 ...
 * @param {String} attributes - the attributes of the element
 */
function xmlLineNoContent(markup, indentation, attributes) {
  if (attributes) {
    return indentation + "<" + markup + " " + attributes + " />\n"
  } else {
    return indentation + "<" + markup + "/>\n"
  }
}
 ...
 */
function Channel(child, url) {
  var elements = []

  if (url) {
    elements.push(function(indentation) {
      return xmlLineNoContent("atom:link", indentation, 'href="' + url + '" rel="self" type="application/rss+xml"')
    })
  }
